...
module.exports = {
    ...
    /**
     *  Custom middleware used by the msite application
     */
    ...
    routeCallback: {
        ...
        partials: function(req, res) {
            ...
            var requestedView = path.join('./', furtherStripped[2], furtherStripped[3]);
        ...
        pageApp: function(req, res) {
            ...
            var requestedFile = './';
            for (var i = 2; i < stripped.length; i++) {
                requestedFile = path.join(requestedFile, stripped[i]);
            }
            var requestedView = requestedFile.split('.')[0];
            res.render(requestedView, function(err, html) {
                if (err) {
                    var file = path.join(config.root, stripped[1], 'mviews', requestedFile);
